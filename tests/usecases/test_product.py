...
from tdd_project.usecases.product import product_use_case
...
async def test_usecases_get_should_return_success(product_id, product_inserted):
    result = await product_use_case.get(id=product_inserted.id)
...
async def test_usecases_get_should_not_found():
    with pytest.raises(NotFoundException) as err:
        await product_use_case.get(id=UUID("1e4f214e-85f7-461a-89d0-a751a32e3bb9"))
...
async def test_usecases_update_should_return_success(product_inserted, product_up):
    ...
    result = await product_use_case.update(id=product_inserted.id, body=product_up)
...
async def test_usecases_delete_should_return_success(product_inserted):
    result = await product_use_case.delete(id=product_inserted.id)
...
async def test_usecases_delete_should_not_found():
    with pytest.raises(NotFoundException) as err:
        await product_use_case.delete(id=UUID("1e4f214e-85f7-461a-89d0-a751a32e3bb9"))
